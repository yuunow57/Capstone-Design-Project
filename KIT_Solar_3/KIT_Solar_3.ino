...
const unsigned long RS232_INTERVAL = 1000; // 1초마다 데이터 요청
...
void setup() {
  ...
  // 릴레이 초기 상태 설정 (모든 출력 OFF, 반전 로직 적용)
  digitalWrite(PILOT_LAMP_GREEN_RED_OUTPUT_PIN, HIGH);  // 파일럿램프 OFF (적색, 반전 수정)
  digitalWrite(FAN_COMMERCIAL_SMPS_OUTPUT_PIN, HIGH);   // 상용팬 OFF (반전 수정)
  digitalWrite(FAN_BATTERY_MODULE_OUTPUT_PIN, HIGH);    // 배터리팬 OFF (반전 수정)
  digitalWrite(HALOGEN_LAMP_OUTPUT_PIN, HIGH);          // 할로겐램프 OFF (반전 수정)
...
void loop()
{
  ...
  switch (command_char)
  {
    ...
    case 'c': // 파일럿램프 RED
      ...
      command_char = 0;
      break;
    case 'd': // 상용/SMPS 선풍기 ON (인터락 기능 추가)
      if(FanBatteryControl == FAN_ON)
      {
        FanBatteryControl = FAN_OFF;  // 이차전지모듈 선풍기 먼저 OFF
        Serial.println("Battery Fan turned OFF due to interlock");
      }
    ...
    case 'e': // 상용/SMPS 선풍기 OFF
      ...
      command_char = 0;
      break;
    case 'f': // 이차전지모듈 선풍기 ON (인터락 기능 추가)
      if(FanCommercialControl == FAN_ON)
      {
        FanCommercialControl = FAN_OFF;  // 상용/SMPS 선풍기 먼저 OFF
        Serial.println("Commercial Fan turned OFF due to interlock");
      }
...
{
  // 인터락 안전 체크: 두 팬이 동시에 ON되는 것을 방지
  if(FanCommercialControl == FAN_ON && FanBatteryControl == FAN_ON)
  {
    // 안전을 위해 둘 다 OFF로 설정
    FanCommercialControl = FAN_OFF;
    FanBatteryControl = FAN_OFF;
    Serial.println("SAFETY: Both fans detected ON - turning OFF both fans!");
  }
  
  // Commercial/SMPS Fan Control (반전 수정)
  digitalWrite(FAN_COMMERCIAL_SMPS_OUTPUT_PIN, (FanCommercialControl == FAN_ON) ? LOW : HIGH);
  
  // Battery Module Fan Control (반전 수정)
  digitalWrite(FAN_BATTERY_MODULE_OUTPUT_PIN, (FanBatteryControl == FAN_ON) ? LOW : HIGH);
}

void updateHalogenLampOutput()
{
  digitalWrite(HALOGEN_LAMP_OUTPUT_PIN, (HalogenLampControl == HALOGEN_LAMP_ON) ? LOW : HIGH);  // 반전 수정
}
...
void processAutomaticControl()
{
  ...
  // 4. 파일럿램프는 수동 제어만 사용 (자동 제어 제거됨)
  // controlPilotLamp(); // 삭제: 수동 제어와 충돌 방지
}
...
void printSystemStatus()
{
  ...
  if(FanCommercialControl == FAN_ON && FanBatteryControl == FAN_ON)
  {
    Serial.println("  ⚠️  WARNING: INTERLOCK VIOLATION DETECTED! Both fans are ON!");
  }
  else if(FanCommercialControl == FAN_ON || FanBatteryControl == FAN_ON)
  {
    Serial.println("  ✅ Interlock OK: Only one fan is active");
  }
  else
  {
    Serial.println("  ✅ Interlock OK: Both fans are OFF");
  }
